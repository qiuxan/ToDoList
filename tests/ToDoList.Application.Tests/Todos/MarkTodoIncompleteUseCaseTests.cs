...
public sealed class MarkTodoIncompleteUseCaseTests
{
    [Fact]
    public async Task ExecuteAsync_WhenTodoExists_MarksTodoIncompleteAndUpdatesUpdatedAt()
    {
        ...
        var useCase = new MarkTodoIncompleteUseCase(
            repository,
            currentUser,
            clock);
        ...
        todo.MarkComplete(new DateTimeOffset(2026, 6, 23, 10, 30, 0, TimeSpan.Zero));
        ...
        Assert.False(storedTodo.IsCompleted);
    ...
    public async Task ExecuteAsync_WhenTodoBelongsToAnotherUser_ReturnsNotFound()
    {
        ...
        otherUserTodo.MarkComplete(new DateTimeOffset(2026, 6, 23, 10, 30, 0, TimeSpan.Zero));
